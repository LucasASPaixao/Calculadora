...
dependencies:
  ...
  workflows:
    # Documentação
    - fill-project-docs.yaml        # preencher docs de projeto existente (pré-requisito dos demais)
    # Desenvolvimento por story
    - story-development-cycle.yaml  # ciclo básico: create → validate → implement → QA
    - development-cycle.yaml        # ciclo avançado (Projeto Bob): PO → Executor → Self-Healing → DevOps
    # Orquestração de épicos
    - epic-orchestration.yaml       # wave-based: executa development-cycle por story em paralelo
    # Brownfield
...
- `*run-workflow fill-project-docs` - Preencher todos os docs de projeto existente (pré-requisito)

**Workflows — Desenvolvimento:**

- `*run-workflow story-development-cycle` - Ciclo completo de uma story (create → implement → QA)
- `*run-workflow development-cycle` - Ciclo avançado por story (PO → Executor → Self-Healing → DevOps)
- `*run-workflow epic-orchestration` - Executar épico completo em waves paralelas
