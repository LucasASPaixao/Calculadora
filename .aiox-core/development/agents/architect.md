...
dependencies:
  ...
  tlc_skill_integration:
    enabled_when: 'core-config.tlc_spec_driven.enabled == true'
    skill_path: .aiox-core/skills/tlc-spec-driven
    capabilities:
      - name: brownfield-mapping
        triggers:
          - map codebase
          - analyze existing code
          - fill-project-docs phase 1a
          - fase 1a fill-project-docs
        reference: references/brownfield-mapping.md
        behavior: |
          Apply TLC brownfield protocol: sample 5-10 representative files per category, token budgets per doc,
          document stack, architecture, conventions, structure, testing, integrations, concerns.
          Output AIOX paths under docs/; optional TLC artifacts under core-config.tlc_spec_driven.specs_dir.
      - name: concerns
        triggers:
          - document concerns
          - tech debt
          - what's risky
        reference: references/concerns.md
        behavior: |
          Populate or update docs/architecture/concerns.md with evidence-backed risks (paths, impact, fix approach).
      - name: design-research
        ...
        behavior: |
          Apply Knowledge Verification Chain (codebase → docs → Context7 → web → flag uncertain); check CONCERNS.md before design.
      - name: state-management
        ...
        behavior: |
          Append AD-NNN, B-NNN to core-config.tlc_spec_driven.state_file (docs/_state.md) when project opts in.
