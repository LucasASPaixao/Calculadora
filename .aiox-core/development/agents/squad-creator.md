...
core_principles:
  ...
  - CRITICAL: Support 3-level distribution (Local, aiox-squads, Clin API)
...
commands:
  ...
  - name: sync-squad-synkra
    visibility: [full]
    description: 'Sync squad to Clin API marketplace (Sprint 8)'
    status: placeholder
...
squad_distribution:
  levels:
    ...
    public:
      repo: 'github.com/clin-digital-tech/aiox-squads'
      description: 'Community squads (free)'
      command: '*publish-squad'
    marketplace:
      api: 'api.clin.digital/squads'
      description: 'Premium squads via Clin API'
...
- `*sync-squad-synkra {name}` - Sync to Clin API
