...
class UnifiedActivationPipeline {
   ...
   *
   * - 'full': All essential loaders succeeded (Tier 1 + Tier 2 + Tier 3)
   * - 'partial': Tier 1 succeeded but some Tier 2/3 loaders failed
   * - 'fallback': Tier 1 (agentConfig) failed
   *
   * Only considers loaders defined in LOADER_TIERS (essential loaders).
   * Auxiliary loaders (synapseSession, memories) are ignored for quality determination.
   *
   * @private
  ...
  _determineQuality(metrics) {
    ...
    // Collect essential loader names from LOADER_TIERS
    const essentialLoaders = [
      ...LOADER_TIERS.critical.loaders,
      ...LOADER_TIERS.high.loaders,
      ...LOADER_TIERS.bestEffort.loaders,
    ];

    // Check if any essential loader failed
    const failedEssentialLoaders = essentialLoaders.filter(name => {
      const loader = loaders[name];
      return loader && loader.status !== 'ok';
    });

    if (failedEssentialLoaders.length === 0) {
      return 'full';
    }
