...
const FIX_MAP = {
  'rules-files': {
    describe(result, { projectRoot, frameworkRoot }) {
      const rulesSource = path.join(frameworkRoot, '.aiox-core', 'rules');
      const rulesTarget = path.join(projectRoot, '.aiox-core', 'rules');
      return `Copy missing canonical rules from ${rulesSource} to ${rulesTarget}`;
    },
    async apply(result, { projectRoot, frameworkRoot }) {
      ...
      return `Copied ${copied} missing canonical rule file(s) into .aiox-core/rules/`;
    },
  },
