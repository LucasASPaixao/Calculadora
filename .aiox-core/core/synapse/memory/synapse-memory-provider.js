...
class SynapseMemoryProvider {
  ...
  _getLoader() {
    ...
    const installed =
      path.join(this._projectDir, '.aiox-core', 'pro', 'memory', 'memory-loader.js');
    const repoRoot =
      path.join(this._projectDir, 'pro', 'memory', 'memory-loader.js');

    try {
      let mod;
      if (existsSync(installed)) {
        mod = require(installed);
      } else if (existsSync(repoRoot)) {
        mod = require(repoRoot);
      } else {
        mod = require('../../../../pro/memory/memory-loader');
      }
      const { MemoryLoader } = mod;
