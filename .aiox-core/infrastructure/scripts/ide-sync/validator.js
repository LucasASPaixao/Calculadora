 ...
 * @param {object} [options] - { skipRedirects: boolean, allowedOrphanFiles: string[] } — skipRedirects: no agent redirects; allowedOrphanFiles: extra files allowed in dir (e.g. Claude-only rules)
 * @returns {object} - Validation result
 */
function validateIdeSync(expectedFiles, targetDir, redirectsConfig, options = {}) {
  const skipRedirects = options.skipRedirects === true;
  const allowedOrphanFiles = options.allowedOrphanFiles || [];
  const result = {
    targetDir,
    missing: [],
    drift: [],
  ...
  // Add redirect filenames to expected (agent sync only)
  if (!skipRedirects) {
  ...
  for (const allow of allowedOrphanFiles) {
    expectedFilenames.add(allow);
  }
  ...
  if (fs.existsSync(targetDir)) {
    try {
      const actualFiles = fs.readdirSync(targetDir).filter(
        f => f.endsWith('.md') || f.endsWith('.mdc') || f.endsWith('.instructions.md')
      );
 ...
 * @param {object} ideConfigs - Map of IDE name to {expectedFiles, targetDir, skipRedirects?}
 * @param {object} redirectsConfig - Redirects configuration
...
function validateAllIdes(ideConfigs, redirectsConfig) {
  ...
  for (const [ideName, config] of Object.entries(ideConfigs)) {
    const ideResult = validateIdeSync(
      ...
      redirectsConfig,
      {
        skipRedirects: config.skipRedirects === true,
        allowedOrphanFiles: config.allowedOrphanFiles || [],
      }
    );
