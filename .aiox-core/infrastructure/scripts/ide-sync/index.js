...
function loadConfig(projectRoot) {
  ...
  const defaultConfig = {
    ...
    rulesSource: '.aiox-core/rules',
    rulesSyncEnabled: true,
    targets: {
      'claude-code': {
        enabled: true,
        path: '.claude/agents',
      ...
      'github-copilot': {
        ...
        rulesFormat: 'github-instructions',
      },
      cursor: {
        enabled: true,
        path: '.cursor/agents',
...
async function commandSync(options) {
  ...
  // Filter IDEs if --ide flag specified OR if selectedIDEs array passed
  ...
  } else if (Array.isArray(options.selectedIDEs) && options.selectedIDEs.length > 0) {
    targetIdes = targetIdes.filter(([name]) => options.selectedIDEs.includes(name));
  }
  ...
  for (const [ideName, ideConfig] of targetIdes) {
    ...
    // AntiGravity: remove obsolete .antigravity/rules/agents/ from older AIOX installs.
    // AntiGravity uses .antigravity/agents/ (root) as its agent directory.
    if (ideName === 'antigravity' && !options.dryRun) {
      const obsolete = path.join(projectRoot, '.antigravity', 'rules', 'agents');
      if (fs.existsSync(obsolete)) {
        try {
          fs.removeSync(obsolete);
          if (!options.quiet) {
            console.log(`${colors.dim}   🧹 Removed obsolete .antigravity/rules/agents/${colors.reset}`);
          }
        } catch (_e) { /* non-fatal */ }
      }
    }

    // Cursor: remove obsolete .cursor/rules/agents/ from older AIOX installs.
    // Cursor agents live in .cursor/agents/ (root); rules/ only holds .mdc governance files.
    if (ideName === 'cursor' && !options.dryRun) {
      const obsolete = path.join(projectRoot, '.cursor', 'rules', 'agents');
      if (fs.existsSync(obsolete)) {
        try {
          fs.removeSync(obsolete);
          if (!options.quiet) {
            console.log(`${colors.dim}   🧹 Removed obsolete .cursor/rules/agents/${colors.reset}`);
          }
        } catch (_e) { /* non-fatal */ }
      }
    }

    // Windsurf: remove obsolete .windsurf/rules/AIOX/ from older AIOX installs.
    // Windsurf agents live in .windsurf/agents/ (root); rules/ only holds governance rules.
    if (ideName === 'windsurf' && !options.dryRun) {
      const obsolete = path.join(projectRoot, '.windsurf', 'rules', 'AIOX');
      if (fs.existsSync(obsolete)) {
        try {
          fs.removeSync(obsolete);
          if (!options.quiet) {
            console.log(`${colors.dim}   🧹 Removed obsolete .windsurf/rules/AIOX/${colors.reset}`);
          }
        } catch (_e) { /* non-fatal */ }
      }
    }

    // Claude Code: remove obsolete .claude/commands/AIOX/ from older AIOX installs.
    // Claude agents live in .claude/agents/ (root); commands/ holds squad subfolders only.
    if (ideName === 'claude-code' && !options.dryRun) {
      const obsolete = path.join(projectRoot, '.claude', 'commands', 'AIOX');
      if (fs.existsSync(obsolete)) {
        try {
          fs.removeSync(obsolete);
          if (!options.quiet) {
            console.log(`${colors.dim}   🧹 Removed obsolete .claude/commands/AIOX/${colors.reset}`);
          }
        } catch (_e) { /* non-fatal */ }
      }
    }
  ...
  // Canonical governance rules → IDE rule folders
  let rulesWritten = 0;
  if (config.rulesSyncEnabled !== false) {
    const rulesDir = path.join(projectRoot, config.rulesSource || '.aiox-core/rules');
    const rules = parseAllRules(rulesDir);
    if (rules.length > 0) {
      if (!options.quiet) {
        console.log(`${colors.dim}Rules source: ${rulesDir} (${rules.length} files)${colors.reset}`);
      }
      for (const [ideName, ideConfig] of targetIdes) {
        if (!ideConfig.enabled || !ideConfig.rulesPath || !ideConfig.rulesFormat) {
          continue;
        }
        if (!options.quiet) {
          console.log(`${colors.cyan}📋 Rules → ${ideName} (${ideConfig.rulesPath})...${colors.reset}`);
        }
        const rr = syncRulesToTarget(rules, projectRoot, ideConfig, options);
        rulesWritten += rr.files.length;
        if (!options.quiet) {
          const errCount = rr.errors.length;
          const status = errCount > 0 ? `${colors.yellow}⚠${colors.reset}` : `${colors.green}✓${colors.reset}`;
          console.log(`   ${status} ${rr.files.length} rule file(s)${errCount > 0 ? `, ${errCount} error(s)` : ''}`);
        }
      }
      if (!options.quiet) {
        console.log('');
  ...
  // Summary
  const agentFilesCount = results.reduce((sum, r) => sum + r.files.length + (r.commandFiles || []).length, 0);
  const totalFiles = agentFilesCount + rulesWritten;
  ...
  if (!options.quiet) {
    ...
    if (options.dryRun) {
      console.log(
        `${colors.yellow}Dry run: ${agentFilesCount} agent outputs + ${totalRedirects} redirects + ${rulesWritten} rules would be written${colors.reset}`
      );
    } else {
      console.log(
        `${colors.green}✅ Sync complete: ${agentFilesCount} agent outputs + ${totalRedirects} redirects + ${rulesWritten} rules${colors.reset}`
      );
    }
...
async function commandValidate(options) {
  ...
  // Canonical rules source validation — verify .aiox-core/rules/ has all expected rule files.
  // Rules are NOT replicated to IDE-specific folders in the framework repo; they live only in
  // the canonical source and are distributed to target projects by the installer.
  if (config.rulesSyncEnabled !== false) {
    const rulesDir = path.join(projectRoot, config.rulesSource || '.aiox-core/rules');
    const expectedBases = Object.keys(RULE_CONFIGS);
    const canonicalExpected = expectedBases.map(base => {
      const filePath = path.join(rulesDir, `${base}.md`);
      const content = fs.existsSync(filePath) ? fs.readFileSync(filePath, 'utf8') : '';
      return { filename: `${base}.md`, content };
    });
    ideConfigs['canonical-rules'] = {
